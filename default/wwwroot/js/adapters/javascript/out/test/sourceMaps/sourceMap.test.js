...
const sourceMap_1 = require("../../src/sourceMaps/sourceMap");
/**
...
suite('SourceMap', () => {
    ...
    const SOURCEMAP_MAPPINGS_JSON = fs.readFileSync(path.resolve(__dirname, 'testData/app.js.map')).toString();
    setup(() => {
    ...
    suite('constructor', () => {
        test('does not crash when sourceRoot is undefined', () => {
            ...
            const sm = new sourceMap_1.SourceMap(GENERATED_PATH, sourceMapJSON, PATH_MAPPING);
            assert(sm);
        });
    });
    suite('.sources', () => {
        ...
        test('sourceMapPathOverrides are respected', () => {
            const sourceMapJSON = getMockSourceMapJSON(SOURCES, SOURCEROOT);
            const sm = new sourceMap_1.SourceMap(GENERATED_PATH, sourceMapJSON, PATH_MAPPING, { '/src/*': testUtils.pathResolve('/project/client/*') });
    ...
    suite('originalPositionFor', () => {
        let sm;
        setup(() => {
            sm = new sourceMap_1.SourceMap(GENERATED_PATH, SOURCEMAP_MAPPINGS_JSON, PATH_MAPPING);
        });
        function getExpectedResult(line, column, source = ABSOLUTE_SOURCES[0]) {
            return {
                line,
                column,
                name: null,
        ...
        test('first line of a method', () => {
            ...
            assert.deepEqual(sm.authoredPositionFor(/*line=*/ 9, /*column=*/ 12), getExpectedResult(/*line=*/ 11, /*column=*/ 12));
        });
        test('private field initializer', () => {
            ...
            assert.deepEqual(sm.authoredPositionFor(/*line=*/ 5, /*column=*/ 17), getExpectedResult(/*line=*/ 4, /*column=*/ 14));
        });
