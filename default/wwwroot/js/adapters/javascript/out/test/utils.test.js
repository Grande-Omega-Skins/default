...
suite('Utils', () => {
    ...
    suite('promiseTimeout()', () => {
        test('when given a promise it fails if the promise never resolves', () => {
            return getUtils().promiseTimeout(new Promise(() => { }), 5).then(() => testUtils.assertFail('This promise should fail'), e => { });
        });
        test('when given a promise it succeeds if the promise resolves', () => {
            return getUtils().promiseTimeout(Promise.resolve('test'), 5).then(result => {
                assert.equal(result, 'test');
            }, e => testUtils.assertFail('This promise should pass'));
        });
        test('when not given a promise it resolves', () => {
            return getUtils().promiseTimeout(null, 5).then(null, () => testUtils.assertFail('This promise should pass'));
        });
    });
    suite('retryAsync()', () => {
        test('when the function passes, it resolves with the value', () => {
            return getUtils().retryAsync(() => Promise.resolve('pass'), /*timeoutMs=*/ 5).then(result => {
        ...
        test('when the function fails, it rejects', () => {
            return getUtils().retryAsync(() => getUtils().errP('fail'), /*timeoutMs=*/ 5)
                .then(() => testUtils.assertFail('This promise should fail'), e => assert.equal(e.message, 'fail'));
        });
    });
    ...
    suite('fileUrlToPath()', () => {
        function testFileUrlToPath(inUrl, expectedUrl) {
    ...
    suite('pathToRegex - case sensitive', () => {
        function testPathToRegex(aPath, expectedRegex) {
    ...
    suite('pathGlobToBlackboxedRegex', () => {
        function testPathGlobToBlackboxedRegex(glob, expected) {
    ...
    suite('makeRegexNotMatchPath/makeRegexMatchPath', () => {
        function testMakeRegexNotMatchPath(regex, noMatchPath, matchPath) {
