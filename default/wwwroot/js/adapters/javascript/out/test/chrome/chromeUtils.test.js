...
suite('ChromeUtils', () => {
    ...
    suite('applyPathMappingsToTargetUrl()', () => {
        ...
        test('decodes uri-encoded characters', () => {
            ...
            assert.equal(getChromeUtils().applyPathMappingsToTargetUrl(url, ROOT_MAPPING), path.join(TEST_WEB_ROOT, segmentWithSpaces, 'script.js'));
        });
        test('matches mappings with uri-encoded characters', () => {
            const segmentWithSpaces = 'path with spaces';
            const escapedSegment = encodeURIComponent(segmentWithSpaces);
            const url = 'http://localhost/' + escapedSegment + '/script.js';
            assert.equal(getChromeUtils().applyPathMappingsToTargetUrl(url, { '/path%20with%20spaces/': TEST_WEB_ROOT }), path.join(TEST_WEB_ROOT, 'script.js'));
        });
        test('resolves webroot-style mapping', () => {
            assert.equal(getChromeUtils().applyPathMappingsToTargetUrl(TEST_TARGET_HTTP_URL, PAGE_MAPPING), TEST_CLIENT_PATH);
        });
        test('resolves webroot-style mapping without trailing slash', () => {
            assert.equal(getChromeUtils().applyPathMappingsToTargetUrl(TEST_TARGET_HTTP_URL, PARTIAL_PAGE_MAPPING), TEST_CLIENT_PATH);
        });
        test('resolves pathMapping for a particular file', () => {
            assert.equal(getChromeUtils().applyPathMappingsToTargetUrl('http://site.com/page.js', FILE_MAPPING), TEST_CLIENT_PATH);
        });
    ...
    suite('getMatchingTargets()', () => {
        ...
        test('really ignores the url protocol', () => {
            const targets = makeTargets('https://outlook.com', 'http://localhost');
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'localhost'), [targets[1]]);
        });
        ...
        test('does not return substring fuzzy match as in pre 0.1.9', () => {
            ...
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'http://localhost/site'), []);
        });
        test('respects one wildcard', () => {
            const targets = makeTargets('http://localhost/site/app', 'http://localhost/site/', 'http://localhost/');
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'localhost/site/*'), [targets[0]]);
        });
        test('respects wildcards with query params', () => {
            ...
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'localhost:3000/site/?*'), [targets[0], targets[1]]);
        });
        test('works with special chars', () => {
            ...
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'http://localhost(foo)/[bar]/?baz'), [targets[0]]);
        });
        test('works with special chars + wildcard', () => {
            ...
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'http://localhost/[bar]/?(*)'), [targets[0], targets[2]]);
        });
        test('matches an ending slash', () => {
            const targets = makeTargets('http://localhost/', 'http://localhost');
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, 'http://localhost'), targets);
        });
        test('works with file://', () => {
            const targets = makeTargets('file:///foo/bar', 'http://localhost');
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, '/foo/bar'), [targets[0]]);
        });
        test('works with file:// + query params', () => {
            const targets = makeTargets('file:///foo/bar?a%3A%2F%2Fb', 'http://localhost');
            assert.deepEqual(chromeUtils.getMatchingTargets(targets, '/foo/bar?a://*'), [targets[0]]);
        });
    });
