...
const nullLogger_1 = require("../../src/nullLogger");
...
suite('ChromeTargetDiscoveryStrategy', () => {
    function getChromeTargetDiscoveryStrategy() {
        const ChromeTargetDiscovery = require(MODULE_UNDER_TEST).ChromeTargetDiscovery;
        return new ChromeTargetDiscovery(new nullLogger_1.NullLogger(), new telemetry_1.NullTelemetryReporter());
    }
    setup(() => {
    ...
    function registerTargetListContents(targetListJSON) {
    ...
    suite('getChromeTargetWebSocketURL()', () => {
        test('rejects promise if getting target list fails', () => {
            ...
            return testUtils.assertPromiseRejected(getChromeTargetDiscoveryStrategy().getTarget(TARGET_ADDRESS, TARGET_PORT));
        });
        ...
        test('rejects promise if no matching targets', () => {
            ...
            registerTargetListContents(JSON.stringify(targets));
            return testUtils.assertPromiseRejected(getChromeTargetDiscoveryStrategy().getTarget(TARGET_ADDRESS, TARGET_PORT, undefined, 'blah.com'));
        });
        test('when no targets have webSocketDebuggerUrl, fails', () => {
            ...
            registerTargetListContents(JSON.stringify(targets));
            return testUtils.assertPromiseRejected(getChromeTargetDiscoveryStrategy().getTarget(TARGET_ADDRESS, TARGET_PORT, undefined, 'localhost/*'));
        });
...
//# sourceMappingURL=chromeTargetDiscoveryStrategy.test.js.map